...
private:
  ...
  typedef std::unique_lock<ceph::shared_mutex> unique_lock;
 ...
 public:
  ...
  struct LingerOp : public RefCountedObject {
    ...
    LingerOp() : linger_id(0),
		 ...
		 register_gen(0), registered(false),
		 canceled(false), on_reg_commit(NULL),
		 on_notify_finish(NULL), notify_result_bl(NULL),
		 watch_context(NULL), session(NULL),
		 register_tid(0), ping_tid(0), map_dne_bound(0),
		 last_force_resend(0) {}
  ...
  struct OSDSession : public RefCountedObject {
    ...
    typedef ceph::shared_lock<ceph::shared_mutex> shared_lock;
    typedef ceph::shunique_lock<ceph::shared_mutex> shunique_lock;
    std::mutex *completion_locks;
    typedef std::unique_lock<std::mutex> unique_completion_lock;
