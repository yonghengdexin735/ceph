			    ...
			    shunique_lock& sul)
{
  ...
  // just to read registered status
...
void Objecter::tick()
{
  ...
  for (map<int,OSDSession*>::iterator siter = osd_sessions.begin();
    ...
    OSDSession::shared_lock l(s->lock);
    bool found = false;
					    ...
					    int *ctx_budget)
{
  ...
  ceph_tid_t tid = _op_submit(op, sul);
...
ceph_tid_t Objecter::_op_submit(Op *op, shunique_lock& sul)
{
  ...
  if ((op->target.flags & CEPH_OSD_FLAG_WRITE) &&
    ...
    _maybe_request_map();
	     ...
	     (_osdmap_full_flag() || _osdmap_pool_full(
	       op->target.base_oloc.pool))) {
...
bool Objecter::target_should_be_paused(op_target_t *t)
{
  ...
  bool pauserd = osdmap->test_flag(CEPH_OSDMAP_PAUSERD);
  bool pausewr = osdmap->test_flag(CEPH_OSDMAP_PAUSEWR) || _osdmap_full_flag()
    || pi->has_flag(pg_pool_t::FLAG_FULL);
