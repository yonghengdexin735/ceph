...
m4_define([_AX_CXX_COMPILE_STDCXX_14_testbody], [[
    ...
    unsigned int x = generic_lambda(1u, 2u);
    double y = generic_lambda(1.4, 2.1);

    #include <memory>
    auto p = std::make_unique<int>(10);

    auto lambda_capture_expressions = [q = std::move(p)] { return *p; };

    auto literal_digit_separators = 1'000;

    using namespace std::literals;

    #include <string>
    std::string str = "std::string literal"s;
